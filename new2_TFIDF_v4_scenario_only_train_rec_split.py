...
預期欄位：
- 訓練檔：artist, song, text, track_id, popularity, human_label
- 推薦檔：artist, song, text, track_id, popularity（human_label 可有可無）

Usage:
    1) 兩個 xlsx 放同一層（或修改 main() 裡路徑）
    2) python 2_TFIDF_v4_scenario_only_train_rec_split.py
"""
...
def load_rec_songs(excel_path: str) -> pd.DataFrame:
    ...
    if "human_label" not in df.columns:
        df["human_label"] = np.nan
    # 若推薦歌庫也有 label（不影響），仍可清理成 label_clean 供畫圖/分析
    df["label_clean"] = df["human_label"].apply(extract_q_label)
    return df
...
def build_vectorizer_and_matrices(
    ...
    回傳：
    - vectorizer
    - X_train_all: train 全部歌的 TF-IDF
    - X_rec: rec 歌庫 TF-IDF
    - df_train_clean, df_rec_clean（多了 clean_text）
    - stop_words
    """
...
# ===== 6) 情境 -> 情緒規則 =====
# Q1開心/興奮, Q2憤怒/緊張, Q3悲傷/痛苦, Q4放鬆/平靜
SCENARIO_TO_EMOTION = {
...
def get_user_text_proba_vector(user_text: str, vectorizer, clf: SVC, stop_words: set) -> np.ndarray:
    ...
    if DEBUG_USER_INPUT:
        ...
        print("X_user.nnz =", X_user.nnz)
        feat_names = vectorizer.get_feature_names_out()
        print("nonzero terms:", [feat_names[i] for i in X_user.indices])
        print("nonzero values:", X_user.data)
...
def main():
    ...
    # 重要：fit_on="both" 可降低 rec 歌庫 OOV；若你想更嚴格可改成 "train"
    ...
    has_label = df_train_clean["label_clean"].notna().any()
    if not has_label:
    ...
    if mode == "1":
        ...
        rec = recommend_top_n(df_rec_clean, emotion_scores_rec, scenario, top_n=15, max_per_artist=2)
        print(f"\n=== 情境：{scenario} 的 Top-N 推薦（來源：{rec_excel}）===")
        print(rec)

    else:
        # 文字輸入：僅允許「情境描述」(scenario/context)
        while True:
            ...
            print(f"\n=== 情境文字輸入：使用機率分佈向量（Q1,Q2,Q3,Q4），模型主預測 {pred} ===")
            print(q.flatten())
